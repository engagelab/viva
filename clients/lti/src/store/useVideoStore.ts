...
async function fetchVideoMetadata(id: string): Promise<VideoData> {
  const payload: APIRequestPayload = {
    method: XHR_REQUEST_TYPE.GET,
    credentials: true,
    route: '/api/video/ltiVideo',
    params: { id },
  }
...
const actions = {
  ...
  getVideoMetadata: async function (): Promise<void> {
    const response = await fetchVideoMetadata(userID)
