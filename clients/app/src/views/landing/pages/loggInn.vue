<template>
  <div>
    <div class="p-4">
      <p class="text-3xl text-viva-korall mb-3">{{ isLoggedIn ? $t('loggedIn') : $t('loggedOut') }}</p>
      <div class="flex flex-row justify-right">
        <Button v-if="!isLoggedIn && browserOk && !appIsOld" logo="feide" class="m-4 focus:outline-none" @click="login()" />
        ...
        <Button v-if="isLoggedIn" class="m-4" @click="listView()">{{$t('MineOpptak')}}</Button>
        <Button v-if="isLoggedIn" class="text-viva-korall m-4" @click="logout()">{{$t('LoggUt')}}</Button>
      </div>
      ...
      <div class="flex flex-row justify-end">
        ...
        <Button
          ...
          class="m-2"
          customWidth="110px"
          @click="validateTransferAccount()"
        >
          <p class="text-xs">{{ $t('AutoriserVIVA') }}</p>
        </Button>
      </div>
    </div>
    <div class="flex flex-row w-full justify-between">
      <div class="pl-16" @click="$emit('slider-back')">
        <SVGSymbol class="fill-current text-viva-korall" applyClasses="w-4 md:w-8" rotation="180" />
      </div>
    </div>
  </div>
</template>
...
<i18n>
{
  "no": {
    ...
    "LoggUt": "Logg ut",
    "GoogleSuite": "Du kan lagre opptak som faller inn i informasjonsklasse begrenset (gule data) på Google Drive med UiO innlogging. Det er behandlingsansvarlig for det datasettet du velger, som bestemmer hvor opptak kan lagres.",
    "UseOfGoogle": "For å kunne ta i bruk UiO Google Drive, må du samtykke til vilkårene for bruk av G Suite for Education (med UiO innlogging). Det kan være du allerede har gitt dette samtykket, og da trenger du selvsagt ikke gi det på nytt. I tillegg må du gi VIVA lov til å lagre opptak på din UiO Google Drive konto på dine vegne. Dette får du spørsmål om første gang du skal lagre et opptak.",
    ...
    "appIsOld": "Viva appen er en eldre versjon, og du må laste ned en ny versjon fra Appstore"
  },
  "en": {
    ...
    "LoggUt": "Log out",
    "GoogleSuite": "You may store recordings that are classified as restricted (yellow) using G Suite for Education with a UiO log in. It is the data controller for the data set you choose who descides where recordings may be stored.",
    "UseOfGoogle": "You have to consent to the terms of use for G Suite for Education (with UiO log in) before you may use this service. You may already have consented. In addition you have to permit VIVA to store recodings in your G Suite Drive on your behalf. You will be asked about this upon your first storage of a recording.",
    ...
    "appIsOld": "Your VIVA app is an older version, please download the latest version from the app store"
  }
}
</i18n>
...
export default {
  ...
  computed: {
    ...mapGetters('general', ['useCordova', 'isLoggedIn', 'appIsOld']),
    browserOk() {
      const AndroidButNotChromeBrowser = /^(?=.*?Android)((?!Gecko\) Chrome).)*$/gi
      const browserNotOk = AndroidButNotChromeBrowser.test(navigator.userAgent)
      return !browserNotOk || this.useCordova
    }
  },
  data() {
    return {
  ...
  methods: {
    ...
    ...mapActions('video', ['initiateTransfer']),
    login() {
