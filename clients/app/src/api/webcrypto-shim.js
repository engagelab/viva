...
!(function (global) {
    ...
    _subtle[m] = function (a, b, c) {
          ...
          if (a === 'jwk') {
            ...
            if (!b.key_ops) {
              b.key_ops =
                b.kty !== 'oct'
                  ? 'd' in b
                    ? ku.filter(isPrvKeyUse)
                    : ku.filter(isPubKeyUse)
                  : ku.slice()
            }
            args[1] = jwk2b(b)
          }
      ...
      if (m === 'generateKey' && ka.name === 'HMAC' && ka.hash) {
        ...
        return _subtle.importKey(
          'raw',
          _crypto.getRandomValues(new Uint8Array((ka.length + 7) >> 3)),
          ka,
          kx,
          ku
        )
      ...
      if (
        (isWebkit || (isIE && (ka.hash || {}).name === 'SHA-1')) &&
        m === 'importKey' &&
        a === 'jwk' &&
        ka.name === 'HMAC' &&
        b.kty === 'oct'
  ...
  ['exportKey', 'wrapKey'].forEach(function (m) {
    ...
    _subtle[m] = function (a, b, c) {
      ...
      if (
        (isWebkit || (isIE && (b.algorithm.hash || {}).name === 'SHA-1')) &&
        m === 'exportKey' &&
        a === 'jwk' &&
      ...
      if (m === 'exportKey' && a === 'jwk') {
        op = op.then(function (k) {
          ...
          if (!k.key_ops) {
            k['key_ops'] =
              b.type === 'public'
                ? b.usages.filter(isPubKeyUse)
                : b.type === 'private'
                ? b.usages.filter(isPrvKeyUse)
                : b.usages.slice()
          }
          return k
        })
