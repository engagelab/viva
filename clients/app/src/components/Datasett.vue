<template>
  <div>
    <div class="pl-4 pt-2 font-vagBold flex flex-row items-center" v-if="selectedDatasett">
      <SVGSymbol
      ...
      <div v-for="(u, i) in completeUtvalgName" :key="i" class="flex flex-row items-center">
    ...
    <div v-if="settings.length === 0" class="flex flex-col items-center w-full px-4 py-2 max-h-1/2">
      <p class="pr-2">
        {{ `${user.username} ${$t('Not registered')}` }}
        <a
          class="text-blue-500"
          href="mailto:ils-kontakt@ils.uio.no"
    ...
    >
      ...
      <NewItem
        v-if="selectedDatasett && selectedUtvalg.length < selectedDatasett.utvalgtPriority.length"
        class="my-2"
        :filter="'[.-\\s]'"
    ...
    <div v-if="utvalgReady" class="flex flex-col items-center w-full">
      <div v-if="selectedDatasett.samtykke == 'samtykke'">
        ...
        <Button class="mt-4 ml-2 my-2 w-64" @click="copyLink()">{{ $t('Kopier lenke til skjema') }}</Button>
      ...
      <div v-else>
        <p class="p-2">{{selectedDatasett.samtykkeHandling[selectedDatasett.samtykke].value}}</p>
      </div>
    </div>
    <div v-if="utvalgReady" class="px-4 pt-4 pb-2 text-2xl flex flex-row justify-end items-center">
  ...
  computed: {
    ...
    utvalgName() {
      if (this.selectedDatasett) {
        return this.selectedDatasett.utvalgtPriority[
